;;; unit-test--setup-entry.el --- Unit test for org-web-track -*- lexical-binding: t -*-
  ...
  (with-temp-buffer
    ...
    (let ((url "https://dummy.com")
      ...
      (should (string= (org-entry-get (point) org-web-track-url)
                       url))
      (re-search-forward "* A-001:")
      (should (equal `(user-error ,(format "No selector found responsible for %s in org-web-track-selectors-alist" url))
                     (should-error (org-web-track-setup-entry url))))
      (should (string= (org-entry-get (point) org-web-track-url)
                       url))
      (re-search-forward "* A-002:")
      (should (equal `(user-error ,(format "No selector found responsible for %s in org-web-track-selectors-alist" url))
                     (should-error (org-web-track-setup-entry url))))
      (should (string= (org-entry-get (point) org-web-track-url)
                       url))
      (re-search-forward "* A-003:")
      (should (equal `(user-error ,(format "No selector found responsible for %s in org-web-track-selectors-alist" url))
                     (should-error (org-web-track-setup-entry url))))
      (should (string= (org-entry-get (point) org-web-track-url)
                       url))
      (re-search-forward "* A-004:")
      (should (equal `(user-error ,(format "No selector found responsible for %s in org-web-track-selectors-alist" url))
                     (should-error (org-web-track-setup-entry url)))))))
...
;;; unit-test--setup-entry.el ends here
